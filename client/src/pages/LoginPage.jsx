...
function LoginPage() {
  ...
  const { signin, validateErrors, isAuthenticated } = UseAuth();
  const navigate =useNavigate()
  const onSubmit = handleSubmit((value) => {
    signin(value);
    console.log(value);
  });
  useEffect(() => {if(isAuthenticated) navigate('/tasks')}, [isAuthenticated]);
  return (
    <div className="flex h-[calc(100vh-100px)] items-center justify-center">
      <div className="bg-zinc-800 max-w-md w-full p-10 rounded-md">
        ...
        <p className="flex gap-x-2 justify-between">
          ...
          <Link to="/register" className="text-sky-700 font-bold">
            {" "}
            Sing up{" "}
          </Link>
        </p>
